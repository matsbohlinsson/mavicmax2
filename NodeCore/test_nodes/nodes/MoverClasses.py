...
class MoverClasses(General):
    input: Input
    output: Output

@dataclass
class Input:
    my_int2: Integer = Integer(value=10)

@dataclass
class Output:
    my_int: Integer = Integer(value=10)

def main_loop(input: Input, output: Output, log: logging.Logger) -> None:
    output.my_int.set(input.my_int2.get())
...
def create_node(plugin_name=plugin_name(__file__), parent=None) -> MoverClasses:
    return MoverClasses(plugin_name=plugin_name, run=main_loop, parent=parent, input=Input(), output=Output())
